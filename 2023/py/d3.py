...
def compare_lines(line0, line1):
    ...
    status = "out" 
    save = False
    running = list(zip(line0,line1))
    for s,n in running:
        print("nl", n,s, status, save)
        # start 
        if (n == "A" or n.isdigit()) and status == "out":
            status = "in"
            new_num.append(n)
            if s == "*":
                save = True
        # end of number
        elif (n == "A") and status == "in":
            status = "out"
            if s == "*":
                save = True
            if save:
               print("SAVE", new_num) 
               final.append("".join(new_num))
               save = False
            new_num = [] 
            # again 
            running.insert(0,(s,n))
        elif n.isdigit()  and status == "in":
            new_num.append(n)
            if s == "*":
                save = True
        elif n == "." and status in ["in","out"]:
            status = "out"
...
def funn(xl,xd):
    if xl.isdigit() and xd == ".":
        return (xl, "A")
    elif xd.isdigit() and xl == ".":
        return ("A", xd)
    else:
        return (xl,xd)
...
def long_nums(line):
    fu = [funn(x0,x1) for x0,x1 in zip(line, line[1:])]
    a,b = list(zip(*fu))
    return [a[0]] + [funnn(x0,x1) for x0,x1 in zip(a[1:],b[:])] + [b[-1]]

mark_misstakes = lambda line:  re.sub(r'[^0-9.]', '*', line)
erase_numbers  = lambda line: re.sub(r'[0-9]', '.', line)
only_numbers_string = lambda line: re.sub(r'[^0-9]', '.', line)

find_star = lambda characters: any(ch == "*" for ch in characters)
line_concatenate = lambda lines : "".join(["*" if find_star(l) else "." for l in zip(*lines)])

misstakes_list = [erase_numbers(mark_misstakes(line)) for line in inputs.split("\n")]
for a in misstakes_list:
    print(a)
print("")
numbers_list = [only_numbers_string(line) for line in inputs.split("\n")]

for a in numbers_list:
    print(a)
print("")
numbers_list = ["".join(long_nums(a)) for a in numbers_list]

for a in numbers_list:
    print(a)
print("")

gether_misstakes = [line_concatenate(a) for a in zip(misstakes_list, misstakes_list[1:], misstakes_list[2:])]
gether_misstakes  = [misstakes_list[1]] + gether_misstakes + [misstakes_list[-2]]
for a in gether_misstakes:
    print(a)
print("")

final = []
for x in zip(gether_misstakes,numbers_list):
    print(x)
    compare_lines(*x)
print(final)
# Remove 'A' and transform to a list of only numbers
# First, filter out 'A' elements
filtered_list = list(filter(lambda element: element != 'A', final))

# Then, remove 'A' from the remaining elements and convert them to integers
number_list = [int(element.replace('A', '')) for element in filtered_list]
